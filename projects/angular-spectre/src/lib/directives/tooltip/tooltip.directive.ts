...
export class NgsTooltipDirective {
  ...
  //@Input('position') tooltipPosition: string;
  ...
  onScroll(event: Event) {
    ...
    let windowYOffset:number = window.pageYOffset, 
    screenHeight:number = window.innerHeight, 
    tooltipYOffset:number = this.ele.nativeElement.offsetTop, 
    elementHeight:number =  this.ele.nativeElement.clientHeight, 
    tooltipHeight:number = parseInt(window.getComputedStyle(this.ele.nativeElement, ':after').height),
    ...
    tooltipWidth:number = parseInt(window.getComputedStyle(this.ele.nativeElement, ':after').width);
    ...
    else if(windowYOffset+screenHeight > tooltipYOffset && windowYOffset+screenHeight < tooltipYOffset+tooltipHeight+elementHeight){
    ...
    else if(windowXOffset+screenWidth > tooltipXOffset && windowXOffset+screenWidth < tooltipXOffset+tooltipWidth+elementWidth){
  ...
  changePosition(position){
    this.renderer.setElementClass(this.ele.nativeElement, 'tooltip-' + this.previous, false);
    this.renderer.setElementClass(this.ele.nativeElement, 'tooltip-' + position, true);
  }
